...
SELECT 
  age_bucket
  ,ROUND(SUM(CASE WHEN activity_type = 'send' THEN 100*time_spent END)/SUM(time_spent), 2) AS send_perc
  ,ROUND(SUM(CASE WHEN activity_type = 'open' THEN 100*time_spent END)/SUM(time_spent), 2) AS send_perc
FROM activities
INNER JOIN age_breakdown
ON
  age_breakdown.user_id = activities.user_id
WHERE
