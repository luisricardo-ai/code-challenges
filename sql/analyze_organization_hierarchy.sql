...
-- # Diff: hard
-- # link: https://leetcode.com/problems/analyze-organization-hierarchy/description/
-- # =================================================================
...
WITH RECURSIVE cte_employee_level AS (
    -- C LEVEL
    SELECT *, 1 AS level
    FROM employees
    ...
    UNION ALL
    -- OTHER EMPLOYEES
    SELECT e.*, c.level + 1 AS level
    FROM employees AS e
    INNER JOIN cte_employee_level AS c
        ON e.manager_id = c.employee_id
), cte_organization_hierarchy AS (
    SELECT 
        DISTINCT c.employee_id, c.employee_name, c.level,  
        ...
        FROM cte_employee_level AS c
        LEFT JOIN LATERAL (
            WITH RECURSIVE cte_manager_level AS (
                SELECT employee_id, manager_id, salary
                FROM employees AS e
                WHERE e.manager_id = c.employee_id
                UNION ALL
                SELECT ee.employee_id, ee.manager_id, ee.salary
                FROM cte_manager_level AS m, Employees AS ee
                WHERE ee.manager_id = m.employee_id
            )
            SELECT * FROM cte_manager_level  
        ) AS e ON 1 = 1
) SELECT * FROM cte_organization_hierarchy
